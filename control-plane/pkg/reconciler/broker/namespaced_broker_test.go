...
func namespacedBrokerReconciliation(t *testing.T, format string, env config.Env) {
	...
	table := TableTest{
		{
			Name: "Reconciled normal - no DLS - first broker in namespace",
			Objects: []runtime.Object{
				NewNamespacedBroker(),
				// TODO: won't be able to mount this
			...
			WantCreates: []runtime.Object{
				NewConfigMapFromContract(&contract.Contract{
					...
					Generation: 0,
				}, &env, WithConfigMapNamespace(BrokerNamespace), WithConfigMapOwnerRef(NewNoControllerOwnerRef(NewBroker()))),
			...
			//		},
			//		Generation: 1,
			//	}, WithConfigMapNamespace(BrokerNamespace), WithConfigMapOwnerRef(NewNoControllerOwnerRef(NewBroker()))),
			//},
	...
	useNamespacedBrokerTestTable(t, table, &env)
}

func NewNoControllerOwnerRef(broker *eventing.Broker) *metav1.OwnerReference {
	ref := kmeta.NewControllerRef(broker)
...
func useNamespacedBrokerTestTable(t *testing.T, table TableTest, env *config.Env) {
	...
	table.Test(t, NewFactory(env, func(ctx context.Context, listers *Listers, env *config.Env, row *TableRow) controller.Reconciler {
		...
		reconciler := &NamespacedReconciler{
			...
			Env:       env,
			Prober:    proberMock,
			IPsLister: prober.NewIPListerWithMapping(),
		}
