...
func (r *Reconciler) reconcileKind(ctx context.Context, trigger *eventing.Trigger) reconciler.Event {
	...
	if isKnativeKafkaBroker, brokerClass := isKnativeKafkaYoloBroker(broker); !isKnativeKafkaBroker {
...
func isKnativeKafkaYoloBroker(broker *eventing.Broker) (bool, string) {
	brokerClass := broker.GetAnnotations()[eventing.BrokerClassAnnotationKey]
	return brokerClass == kafka.BrokerClassYolo, brokerClass
}
