...
func (r *Reconciler) reconcileKind(ctx context.Context, broker *eventing.Broker) reconciler.Event {
	...
	// TODO: if not exists, create them
	// TODO: if exists but not ready, wait for them
	if !r.IsReceiverRunning(broker.Namespace) || !r.IsDispatcherRunning(broker.Namespace) {
		// TODO: only use this status if the dataplane exists but not running
	...
	// TODO: Should useNamespaceInConfigmap be true?
	...
	contractConfigMap, err := r.GetOrCreateContractConfigMap(ctx, broker.Namespace)
	if err != nil {
	...
	if err := r.UpdateReceiverPodsAnnotation(ctx, broker.Namespace, logger, ct.Generation); err != nil {
	...
	if err := r.UpdateDispatcherPodsAnnotation(ctx, broker.Namespace, logger, ct.Generation); err != nil {
...
func (r *Reconciler) finalizeKind(ctx context.Context, broker *eventing.Broker) reconciler.Event {
	...
	// TODO: delete the dataplane if there are no brokers left in the namespace
